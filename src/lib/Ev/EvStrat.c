...
void ES_Population_Make_New_Generation( population_t *parent_population,
										population_t *child_population,
                                        population_t *new_population,
										RNG          *rng)

/* ES generally applies selection pressure based on the objective function AFTER children
   have been generated.  Unlike SGA, which selects parents based on fitness, ES selects
   parents at random, generates children, and then chooses who lives from that set using
   selection.  There's lots of ways of doing this, of course.  Most ES's use tournaments
   between children or parents and children.  Naturally there's LOTS of way of doing those
   pairings. 
 
   Here I chose a simple method.  You have a parent population of N members and a child 
   population of N members.  I run N tournaments, each with a randomly selected parent and
   a randomly selected child.  The winner makes it into one of the N slots of a new 
   population.  
 
   It might be an interesting exercise to think about the possible effects of running that
   tournament with only children or with parent/child and child/child tournament pairings.
 
   Oh... yes... this is MUCH more easy to implement thatn fitness proportional selection...
   _much_
 
   Another note, here we're set up to maximize the objective function.  If we wanted to 
   minimize, we'd just turn flip the sense of the comparisons over in each tournament run 
*/


{   void ES_Population_Compute_Fitness();
	int tournament_count;
	int parent_index;
	int child_index;
	
	// Let's make sure we have fresh error scores computed... 
	
	ES_Population_Compute_Fitness(parent_population);
	ES_Population_Compute_Fitness(child_population);
	
	// Now let's make a new population by running one tournament per member of 
	// a population
	
	for (tournament_count = 0; tournament_count < (*parent_population).member_count; tournament_count++)
	    { // choose random dudes from each of the parent and child popuations
			parent_index = (int)floor(rng_uniform(rng, 0.0, (double)(*parent_population).member_count));
			child_index  = (int)floor(rng_uniform(rng, 0.0, (double)(*child_population).member_count));
	
		  // run the tournament
			
		  if (((*parent_population).member[parent_index]).fitness >  ((*child_population).member[child_index]).fitness)
			  ES_Genome_Copy( &((*parent_population).member[parent_index]),&((*new_population).member[tournament_count]));
		  else
			  ES_Genome_Copy( &((*child_population).member[child_index]),&((*new_population).member[tournament_count]));
...
int main()
{
	...
	* Allocate memory for three populations... *
	ES_Population_Malloc(&PARENT_POP,1000,2);  
	ES_Population_Malloc(&CHILD_POP,1000,2); 
	ES_Population_Malloc(&NEW_POP,1000,2);
	...
	* Begin a simplistic fixed size loop of evolution... it's just a count people ;) *
	...
	for (generation_count = 0; generation_count < 5000; generation_count++)
	     { ES_Population_Copy(&PARENT_POP, &CHILD_POP);
		   ES_Population_Mutate(&CHILD_POP, random_number_generator);	
		   ES_Population_Make_New_Generation(&PARENT_POP, &CHILD_POP, &NEW_POP, random_number_generator);
		   ES_Population_Copy(&NEW_POP, &PARENT_POP);
		 }
			 
	ES_Population_Print(&PARENT_POP);
}
*/	
